...
```text
1. 修正 evaluator: causal descriptor, episode split, train-only normalization
2. 补 artifact writer/loader、retrieval montage 和几何扰动评估
3. 用 Package Scan v6 做 smoke,再在公开数据完成 Gate 1/2
4. 定义 StateBatch/CodeMeasurement/ModeBatch typed interfaces
5. 实现 mask program 与防泄漏/梯度路由单测
6. 实现 continuous C0 和 FrozenRQAdapter,完成 C0/C1
7. 实现独立 WorldExpert 与 hierarchical heads,完成 C2
8. 逐项验证 structured masks
9. 最后才接入可选 recurrent/TTT MemoryPort
```
