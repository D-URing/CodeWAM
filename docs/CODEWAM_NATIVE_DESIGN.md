...
```text
机器人控制中,能否用离散、可预测、行为相关的 visual state code
为连续视觉 latent 建立稳定的状态坐标系,让 policy、dynamics、diagnostics
和 planning 都能围绕这个坐标系组织?
```

如果这个判断成立,CodeWAM 的价值不是多一个 regularizer,也不是用 code 替代 latent,
而是建立一个可以被动作模型真正使用的 hybrid 状态空间:

```text
continuous visual latent = 精确几何和操作细节
...
```text
H1: 机器人操作中存在比连续视觉 latent 更紧凑的离散 visual state coordinates。
H2: 这些 code coordinates 可以通过未来预测、动作预测和扰动诊断被训练成 action-relevant。
H3: 一旦 state code 足够稳定,policy/dynamics/planning 可以围绕它组织,但动作精调仍保留 continuous latent。
...
```text
observation -> continuous visual latent + frozen visual state codes
latent + state code + proprio + task + history -> action
latent + state code + action -> next state code / next latent proxy
future state code -> value / stage / success / contact
```

重点变化:

- Code 从辅助 token 变成状态坐标,但不吞掉 continuous latent。
...
- code-only bottleneck 丢失关键几何细节,导致动作抖动或接触失败。
...
```text
CodeWAM-hybrid native v0:

1. observation -> continuous Wan-VAE latent and optional video-DiT hidden feature
2. offline train/version grid RQ codebook, then freeze encoder + centers and export fixed codes
3. continuous latent + code history + proprio history + task -> small belief/action transformer
4. belief -> action chunk, with continuous latent retained for precision
5. belief + action -> next frozen code / future-code distribution
6. future code -> stage/contact/success proxy
7. diagnostics: latent-only, code-only, latent+code, proprio-only, perturbation, nearest-neighbor, copy baseline
```
